...
(function () {
  ...
  PIXI.DisplayObject.prototype = {
    ...
    generateTexture(resolution, scaleMode, renderer) {
      ...
      const renderTexture = new Phaser.RenderTexture(
        this.game,
        bounds.width | 0,
        bounds.height | 0,
        renderer,
    ...
    _renderCachedSprite(renderSession) {
      ...
      if (renderSession.gl) {
        PIXI.Sprite.prototype._renderWebGL.call(
          this._cachedSprite,
          renderSession,
        );
      } else {
        PIXI.Sprite.prototype._renderCanvas.call(
          this._cachedSprite,
          renderSession,
        );
      }
    ...
    _generateCachedSprite() {
      ...
      if (!this._cachedSprite) {
        ...
        if (
          ...
          PIXI._enableMultiTextureToggle
        ) {
      ...
      this._cachedSprite.texture.render(
        this,
        PIXI.DisplayObject._tempMatrix,
        true,
      );
  ...
  PIXI.DisplayObjectContainer.prototype.constructor =
 ...
 */
  ...
  PIXI.DisplayObjectContainer.prototype.getBounds = function (targetCoordinateSpace) {
    const isTargetCoordinateSpaceDisplayObject =
      ...
      targetCoordinateSpace instanceof PIXI.DisplayObject;
  ...
  PIXI.Sprite = function (texture) {
    ...
    this.anchor = new PIXI.Point(
      PIXI.Sprite.defaultAnchor.x,
      PIXI.Sprite.defaultAnchor.y,
    );
  ...
  PIXI.Sprite.prototype._renderCanvas = function (renderSession, matrix) {
    ...
    const resolution =
      this.texture.baseTexture.resolution / renderSession.resolution;
    ...
    let dx = this.texture.trim
      ? this.texture.trim.x - this.anchor.x * this.texture.trim.width
      : this.anchor.x * -this.texture.frame.width;
    let dy = this.texture.trim
      ? this.texture.trim.y - this.anchor.y * this.texture.trim.height
      : this.anchor.y * -this.texture.frame.height;

    let tx = wt.tx * renderSession.resolution + renderSession.shakeX;
    let ty = wt.ty * renderSession.resolution + renderSession.shakeY;

    let cw = this.texture.crop.width;
    let ch = this.texture.crop.height;
  ...
  PIXI.PixiShader.prototype.initMultitexShader = function () {
    ...
    for (let index = 1; index < this.MAX_TEXTURES; ++index) {
      dynamicIfs +=
        `\tif (vTextureIndex == ${
          index
    ...
    const program = PIXI.compileProgram(
      gl,
      this.vertexSrc || PIXI.PixiShader.defaultVertexSrc,
      this.fragmentSrc,
    );
  ...
  PIXI.ComplexPrimitiveShader.prototype.constructor =
  ...
  PIXI.WebGLRenderer.prototype.setTexturePriority = function (textureNameCollection) {
    ...
    const clampPot = function (potSize) {
      --potSize;
      potSize |= potSize >> 1;
      potSize |= potSize >> 2;
      potSize |= potSize >> 4;
    ...
    const imageCache = this.game.cache._cache.image;
    let imageName = null;
    ...
    for (let i = 0; i < this.currentBatchedTextures.length; i++) {
    ...
    let maxTextureAvailableSpace =
      maxTextureSize - clampPot(Math.max(this.width, this.height));
    this.currentBatchedTextures.length = 0;
    // We start from 1 because framebuffer texture uses unit 0.
    for (let j = 0; j < textureNameCollection.length; ++j) {
      ...
      const base = imageCache[imageName].base;
   ...
   */
  PIXI.WebGLRenderer.prototype.renderDisplayObject = function (
    displayObject,
    projection,
    buffer,
    matrix,
  ...
  PIXI.WebGLRenderer.prototype.updateCompressedTexture = function (texture) {
    ...
    const gl = this.gl;
    const textureMetaData = texture.source;
  ...
  PIXI.WebGLMaskManager.prototype.pushMask = function (maskData, renderSession) {
    ...
    renderSession.stencilManager.pushStencil(
      maskData,
      maskData._webGL[gl.id].data[0],
      renderSession,
    );
  };
  ...
  PIXI.WebGLMaskManager.prototype.popMask = function (maskData, renderSession) {
    ...
    renderSession.stencilManager.popStencil(
      maskData,
      maskData._webGL[gl.id].data[0],
      renderSession,
    );
   ...
   * @param renderSession {Object}
   */
  PIXI.WebGLStencilManager.prototype.pushStencil = function (
    graphics,
    webGLData,
    renderSession,
  ) {
   ...
   * @param renderSession {Object}
   */
  PIXI.WebGLStencilManager.prototype.bindGraphics = function (
    graphics,
    webGLData,
    renderSession,
  ) {
    ...
    if (webGLData.mode === 1) {
      ...
      gl.uniform3fv(
        shader.tintColor,
        Phaser.Color.hexToRGBArray(graphics.tint),
      );
   ...
   * @param renderSession {Object}
   */
  PIXI.WebGLStencilManager.prototype.popStencil = function (
    graphics,
    webGLData,
    renderSession,
  ) {
  ...
  PIXI.WebGLShaderManager.prototype.setContext = function (gl) {
    ...
    this.creatureShader = PIXI.CreatureShader
      ? new PIXI.CreatureShader(gl)
      : null;
  ...
  PIXI.WebGLSpriteBatch = function (game) {
    ...
    const numVerts = this.vertexSize * this.size * 4;
  ...
  PIXI.WebGLSpriteBatch.prototype.setContext = function (gl) {
    ...
    if (PIXI._enableMultiTextureToggle) {
      ...
      this.defaultShader = new Phaser.Filter(this.game, undefined, [
  ...
  PIXI.WebGLSpriteBatch.prototype.render = function (sprite, matrix) {
    ...
    let i = this.currentBatchSize * this.vertexSize; // 4 * this.vertSize;
    const tiOffset = this.currentBatchSize * 4;
    const resolution = texture.baseTexture.resolution;
    const textureIndex = texture.baseTexture.textureIndex;

    let a = wt.a / resolution;
    let b = wt.b / resolution;
    let c = wt.c / resolution;
    let d = wt.d / resolution;
    let tx = wt.tx;
    let ty = wt.ty;

    const cw = texture.crop.width;
    const ch = texture.crop.height;
  ...
  PIXI.WebGLSpriteBatch.prototype.renderTilingSprite = function (sprite) {
    ...
    const baseTexture = texture.baseTexture;
    const gl = this.gl;
    const textureIndex = sprite.texture.baseTexture.textureIndex;
    ...
    const scaleX =
      sprite.width / w / (sprite.tileScale.x * sprite.tileScaleOffset.x);
    const scaleY =
      sprite.height / h / (sprite.tileScale.y * sprite.tileScaleOffset.y);
  ...
  PIXI.WebGLSpriteBatch.prototype.flush = function () {
    ...
    if (this.dirty) {
      ...
      const stride = this.vertexSize; // this.vertSize * 4;
      gl.vertexAttribPointer(
        shader.aVertexPosition,
        2,
        gl.FLOAT,
    ...
    if (this.currentBatchSize > this.size * 0.5) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexBuffer);
      const view = this.positions.subarray(
        0,
    ...
    for (let i = 0, j = this.currentBatchSize; i < j; i++) {
      ...
      //
      if (
        /* (currentBaseTexture != nextTexture && !skip) || */
        blendSwap ||
        shaderSwap
      ) {
   ...
   */
  PIXI.WebGLSpriteBatch.prototype.renderBatch = function (
    texture,
    size,
    startIndex,
  ) {
   ...
   * @param renderSession {Object}
   */
  PIXI.WebGLFastSpriteBatch.prototype.begin = function (
    spriteBatch,
    renderSession,
  ) {
    this.renderSession = renderSession;
  ...
  PIXI.WebGLFastSpriteBatch.prototype.render = function (spriteBatch) {
    ...
    // check blend mode
    if (
      sprite.blendMode !== this.renderSession.blendModeManager.currentBlendMode
    ) {
      this.flush();
  ...
  PIXI.WebGLFilterManager.prototype.pushFilter = function (filterBlock) {
    ...
    if (!texture) {
      texture = new PIXI.FilterTexture(
        this.gl,
    ...
    gl.viewport(
      0,
      0,
      filterArea.width * this.renderSession.resolution,
      filterArea.height * this.renderSession.resolution,
    );
  ...
  PIXI.WebGLFilterManager.prototype.popFilter = function () {
    ...
    if (filterBlock.filterPasses.length > 1) {
      ...
      if (!outputTexture) {
        outputTexture = new PIXI.FilterTexture(
          this.gl,
          this.width * this.renderSession.resolution,
          this.height * this.renderSession.resolution,
        );
    ...
    gl.viewport(
      0,
      0,
      sizeX * this.renderSession.resolution,
      sizeY * this.renderSession.resolution,
    );
   ...
   */
  PIXI.WebGLFilterManager.prototype.applyFilterPass = function (
    filter,
    filterArea,
    width,
    height,
  ...
  PIXI.FilterTexture = function (gl, width, height, scaleMode, textureUnit) {
     ...
     * @type Any
     */
    this.frameBuffer = _CreateFramebuffer.call(
      this,
      gl,
      width,
      height,
   ...
   */
  PIXI.CanvasMaskManager.prototype.pushMask = function (
    maskData,
    renderSession,
  ) {
  ...
  PIXI.CanvasTinter.getTintedTexture = function (sprite, color) {
    const canvas = sprite.tintedTexture || Phaser.CanvasPool.create(this);
  ...
  PIXI.CanvasTinter.tintWithPerPixel = function (texture, color, canvas) {
    ...
    const rgbValues = Phaser.Color.hexToRGBArray(color);
    let r = rgbValues[0],
      g = rgbValues[1],
      b = rgbValues[2];
  ...
  PIXI.CanvasRenderer.prototype.resize = function (width, height) {
    ...
    if (this.renderSession.smoothProperty) {
      this.context[this.renderSession.smoothProperty] =
        this.renderSession.scaleMode === PIXI.scaleModes.LINEAR;
    }
  };
  ...
  PIXI.CanvasRenderer.prototype.mapBlendModes = function () {
    if (!PIXI.blendModesCanvas) {
      const b = [];
      const modes = PIXI.blendModes;
      const useNew = this.game.device.canUseMultiply;
